...
function Recieve() {
  ...
  const fetchData = () => {
    ...
    axios({
      method: "get",
      url: "http://127.0.0.1:5174/api/PostMenu",
      headers: {
        "Content-Type": "application/json",
        Authorization: "Bearer " + localStorage.getItem("token"),
      },
  ...
  return (
    <div className="w-screen bg-base  min-h-screen bg-repeat-y bg-cover pb-20 font-prom">
      ...
      <div className=" my-14">
        <button
          href="/"
          className="fixed z-20 top-20 right-10 sm:right-6 border-t-4 font-bold border-b-4 border-r-4 border-orange-600 bg-amber-400 w-10 h-10  rounded-r-2xl  flex justify-center items-center text-white text-lg sm:text-xl hover:bg-orange-600  duration-300"
        >
          <a href="/Post">
            <FiBell />
          </a>
